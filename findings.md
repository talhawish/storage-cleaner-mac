...
Last updated: June 25, 2026 (M9, N1 fixed)
...
Top items:
1. **`~/.gradle/caches` double-counted (N2)** — Remove `home(".gradle/caches")` from `DependencyPaths.Android.cacheDirs`. 2-minute fix.
2. **`ForEach` with `\.self` on URL arrays (M6)** — Crash risk with duplicate URLs.
3. **Docker finding has no `filePaths` (N3)** — Empty paths when daemon is reachable; Colima/OrbStack missed.
4. **`Non-Sendable` `Process` in `@Sendable` closure (M7)** — 3 files affected.
